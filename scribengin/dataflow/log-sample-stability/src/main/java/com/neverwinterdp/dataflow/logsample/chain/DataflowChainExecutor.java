...
public class DataflowChainExecutor extends Executor {
  ...
  public DataflowChainExecutor(ScribenginShell shell,LogSampleChainConfig config) throws Exception {
    ...
    for(int i = 0; i < dflChainconfig.getDescriptors().size(); i++) {
      ...
      descriptor.setNumberOfWorkers(config.dataflowNumOfWorker);
      descriptor.setNumberOfExecutorsPerWorker(config.dataflowNumOfExecutorPerWorker);
      if(config.dataflowTaskDedicatedExecutor) {
