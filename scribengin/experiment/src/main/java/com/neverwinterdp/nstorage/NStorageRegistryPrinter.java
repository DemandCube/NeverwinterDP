...
public class NStorageRegistryPrinter {
  private Appendable             out;
  private NStorageRegistry segStorageReg;
  
  public NStorageRegistryPrinter(Appendable out, NStorageRegistry segStorageReg) {
    this.out           = out;
  ...
  public void printWriters() throws IOException, RegistryException {
    ...
    for(int i = 0; i < allWriters.size(); i++) {
      NStorageWriterDescriptor writer = segStorageReg.getWriter(allWriters.get(i));
