...
public class TrackingReport {
  ...
  public TrackingReport(String groupId, int numOfMessage, int progress, int lostCount, int duplicatedCount) {
  ...
  static public String getFormattedReport(String title, List<TrackingReport> reports) {
    TabularFormater formater = new TabularFormater("Group Id", "Num Of Message", "Progress", "Lost", "Duplicated");
    formater.setTitle(title);
    for(TrackingReport report : reports) {
      formater.addRow(report.getGroupId(), report.getNumOfMessage(), report.getProgress(), report.getLostCount(), report.getDuplicatedCount());
    }
